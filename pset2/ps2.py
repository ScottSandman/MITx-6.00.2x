...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        new_pos = pos.getNewPosition(self.getRobotDirection(), self.speed)
        if self.room.isPositionInRoom(new_pos):
            self.setRobotPosition(new_pos)
            if not self.room.isTileCleaned(int(new_pos.getX()), int(new_pos.getY())):
...
#print(runSimulation(3, 1.0, 15, 15, 1, 1, StandardRobot))
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        pos = self.getRobotPosition()
        new_pos = pos.getNewPosition(self.getRobotDirection(), self.speed)
        new_ang = random.randint(0, 359)
        if self.room.isPositionInRoom(new_pos):
            self.setRobotPosition(new_pos)
            self.setRobotDirection(new_ang)
            if not self.room.isTileCleaned(int(new_pos.getX()), int(new_pos.getY())):
                self.room.cleanTileAtPosition(new_pos)
        else:
            self.setRobotDirection(new_ang)
...
showPlot1('Standard vs. Random', 'number robots', 'Time Steps' )
...
#showPlot2('Room Size', 'Room Size', 'Time Steps')
# === Problem 6
